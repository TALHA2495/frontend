...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch('https://backend-topaz-one-89.vercel.app/api/auth/login', {
        method: 'POST',
        headers: {
  ...
  const register = async (name, email, password) => {
    try {
      const response = await fetch('https://backend-topaz-one-89.vercel.app/api/auth/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
